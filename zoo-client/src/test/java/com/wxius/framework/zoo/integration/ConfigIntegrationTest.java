 ...
 *
 */
package com.wxius.framework.zoo.integration;
...
import com.wxius.framework.zoo.BaseIntegrationTest;
...
//import com.wxius.framework.zoo.infrastructure.RemoteConfigLongPollService;
...
//    ZooConfig zooConfig = assembleZooConfig(ImmutableMap.of(someKey, someValue));
//    ContextHandler handler = mockConfigServerHandler(HttpServletResponse.SC_OK, zooConfig);
...
//    ZooConfig zooConfig = assembleZooConfig(ImmutableMap.copyOf(configurations));
//    ContextHandler handler = mockConfigServerHandler(HttpServletResponse.SC_OK, zooConfig);
//    startServerWithHandlers(handler);
...
//    ZooConfig zooConfig = assembleZooConfig(ImmutableMap.of(someKey, anotherValue));
//    ContextHandler handler = mockConfigServerHandler(HttpServletResponse.SC_OK, zooConfig);
//    startServerWithHandlers(handler);
...
//  }
  ...
  public void testGetConfigWithNoLocalFileAndRemoteMetaServiceRetry() throws Exception {
    ...
    String someValue = "someValue";
//    ZooConfig zooConfig = assembleZooConfig(ImmutableMap.of(someKey, someValue));
//    ContextHandler configHandler = mockConfigServerHandler(HttpServletResponse.SC_OK, zooConfig);
...
//    ContextHandler handler =
//        mockConfigServerHandler(HttpServletResponse.SC_OK, zooConfig, failedAtFirstTime);
//    startServerWithHandlers(handler);
...
//    ZooConfig zooConfig = assembleZooConfig(configurations);
//    ContextHandler handler = mockConfigServerHandler(HttpServletResponse.SC_OK, zooConfig);
//    startServerWithHandlers(handler);
    ...
    config.addChangeListener(new DatumChangeListener() {
...
//    zooConfig.getConfigurations().put(someKey, anotherValue);
...
//            Lists.newArrayList(
//                new ZooConfigNotification(zooConfig.getNamespaceName(), someNotificationId)),
//            false);
//
...
//    startServerWithHandlers(configHandler, pollHandler);
    ...
    Datum someOtherConfig = ConfigService.getConfig(someOtherNamespace);
    Datum config = ConfigService.getAppConfig();
    assertEquals(someValue, config.getProperty(someKey, null));
...
//            Lists.newArrayList(
//                new ZooConfigNotification(zooConfig.getNamespaceName(), someNotificationId),
//                new ZooConfigNotification(someOtherNamespace, someNotificationId)),
//            false);
...
//    startServerWithHandlers(configHandler, pollHandler);
    ...
    someOtherConfig.addChangeListener(new DatumChangeListener() {
      @Override
      public void onChange(ConfigChangeEvent changeEvent) {
...
//      final List<ZooConfigNotification> result,
...
//        response.getWriter().println(gson.toJson(result));
  ...
  private ContextHandler mockConfigServerHandler(final int statusCode,
//                                                 final ZooConfig result,
      final boolean failedAtFirstTime) {
...
//  private ContextHandler mockConfigServerHandler(int statusCode, ZooConfig result) {
...
//    ZooConfig zooConfig =
//        new ZooConfig(someAppId, someClusterName, defaultNamespace, someReleaseKey);
//
