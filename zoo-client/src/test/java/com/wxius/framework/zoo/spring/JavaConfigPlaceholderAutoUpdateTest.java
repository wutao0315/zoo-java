...
import com.wxius.framework.zoo.spring.JavaConfigPlaceholderTest.JsonBean;
//import com.wxius.framework.zoo.spring.XmlConfigPlaceholderTest.TestXmlBean;
...
//    config.onRepositoryChange(ConfigConsts.NAMESPACE_APPLICATION, newProperties);
  ...
  public void testAutoUpdateWithMultipleNamespaces() throws Exception {
    ...
    Properties fxZooProperties = assembleProperties(BATCH_PROPERTY, String.valueOf(initialBatch));

//    SimpleConfig applicationConfig = prepareConfig(ConfigConsts.NAMESPACE_APPLICATION, applicationProperties);
//    SimpleConfig fxZooConfig = prepareConfig(FX_ZOO_NAMESPACE, fxZooProperties);
...
//    applicationConfig.onRepositoryChange(ConfigConsts.NAMESPACE_APPLICATION, newApplicationProperties);
    ...
    Properties newFxZooProperties = assembleProperties(BATCH_PROPERTY, String.valueOf(newBatch));

//    fxZooConfig.onRepositoryChange(FX_ZOO_NAMESPACE, newFxZooProperties);
...
//    SimpleConfig fxZooConfig = prepareConfig(FX_ZOO_NAMESPACE, fxZooProperties);
    ...
    Properties newFxZooProperties = assembleProperties(TIMEOUT_PROPERTY, String.valueOf(someNewTimeout),
...
//    prepareConfig(FX_ZOO_NAMESPACE, fxZooProperties);
...
//    SimpleConfig config = prepareConfig(ConfigConsts.NAMESPACE_APPLICATION, properties);
    ...
    TestZooJsonValue bean = context.getBean(TestZooJsonValue.class);
...
//    SimpleConfig config = prepareConfig(ConfigConsts.NAMESPACE_APPLICATION, properties);
    ...
    TestZooJsonValueWithDefaultValue bean = context.getBean(TestZooJsonValueWithDefaultValue.class);
...
//    config.onRepositoryChange(ConfigConsts.NAMESPACE_APPLICATION, newProperties);
  ...
  static class AppConfig10 {
    ...
    @Bean
    TestZooJsonValue testZooJsonValue() {
      return new TestZooJsonValue();
    }
  }
  ...
  static class AppConfig11 {
    ...
    @Bean
    TestZooJsonValueWithDefaultValue testZooJsonValue() {
      return new TestZooJsonValueWithDefaultValue();
    }
  }
  ...
  static class TestZooJsonValueWithDefaultValue {

    @ZooJsonValue("${jsonProperty:{\"a\":\"defaultString\", \"b\":1}}")
    private JsonBean jsonBean;
