...
import com.wxius.framework.zoo.util.function.Functions;
...
public abstract class AbstractDatum implements Datum {
  private static final Logger logger = LoggerFactory.getLogger(AbstractDatum.class);

  private static final ExecutorService m_executorService;

  private final List<DatumChangeListener> m_listeners = Lists.newCopyOnWriteArrayList();
  private final Map<DatumChangeListener, Set<String>> m_interestedKeys = Maps.newConcurrentMap();
  private final Map<DatumChangeListener, Set<String>> m_interestedKeyPrefixes = Maps.newConcurrentMap();
  ...
  static {
    m_executorService = Executors.newCachedThreadPool(ZooThreadFactory
        .create("Config", true));
  }
  ...
  public AbstractDatum() {
    ...
    propertiesFactory = ZooInjector.getInstance(PropertiesFactory.class);
  }
  ...
  @Override
  public void addChangeListener(DatumChangeListener listener, Set<String> interestedKeys) {
    addChangeListener(listener, interestedKeys, null);
  }

  @Override
  public void addChangeListener(DatumChangeListener listener, Set<String> interestedKeys, Set<String> interestedKeyPrefixes) {
  ...
  @Override
  public boolean removeChangeListener(DatumChangeListener listener) {
  ...
  public Integer getIntProperty(String key, Integer defaultValue) {
    try {
      ...
      Tracer.logError(new ZooException(
   ...
   */
  protected void fireConfigChange(String name, Map<String, ConfigChange> changes) {
    final Set<String> changedKeys = changes.keySet();
    final List<DatumChangeListener> listeners = this.findMatchedConfigChangeListeners(changedKeys);

    // notify those listeners
    for (DatumChangeListener listener : listeners) {
      ...
      InterestedConfigChangeEvent interestedConfigChangeEvent = new InterestedConfigChangeEvent(
              name, changes, interestedChangedKeys);
  ...
  protected void fireConfigChange(final ConfigChangeEvent changeEvent) {
    final List<DatumChangeListener> listeners = this
  ...
  private List<DatumChangeListener> findMatchedConfigChangeListeners(Set<String> changedKeys) {
    final List<DatumChangeListener> configChangeListeners = new ArrayList<>();
    for (DatumChangeListener configChangeListener : this.m_listeners) {
  ...
  private void notifyAsync(final DatumChangeListener listener, final ConfigChangeEvent changeEvent) {
    m_executorService.submit(new Runnable() {
      @Override
      public void run() {
        String listenerName = listener.getClass().getName();
        Transaction transaction = Tracer.newTransaction("Zoo.ConfigChangeListener", listenerName);
        try {
  ...
  private boolean isConfigChangeListenerInterested(DatumChangeListener configChangeListener, Set<String> changedKeys) {
  ...
  private Set<String> resolveInterestedChangedKeys(DatumChangeListener configChangeListener, Set<String> changedKeys) {
